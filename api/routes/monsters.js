...
router.get("/rise/monsters", function (req, res) {
	try {
    ...
    const questData = [];
		...
		for (const i in quests) {
			quests[i]["id"] = parser.toURL(quests[i].primary_monster);
...
router.get(
	"/rise/monsters/:name/:weapon/:ruleset/:platform",
	function (req, res) {
		try {
          ...
          for (const k in quests) {
            const id = parser.toURL(quests[k].primary_monster);
            const questId = parser.toURL(quests[k].name);

            if (params.name == id && questName == questId) {
